...
<div class="story-blocks" v-show="isStoryBlocksFetched">
    <div class="story-content">
        <div class="story-block {{storyblock.class}}" v-for="storyblock in storyblocks">
            <a v-link="{name: 'category', params: {title: storyblock.title}}" aria-label="{{storyblock.title}}">
                <h6 class="block_title heading{{storyblock.order}}"><span><b class="{{storyblock.icon}}"></b>{{storyblock.title}}</span></h6>
            </a>
            <ul>
                <li v-for="story in storyblock.list">
                    <a v-link="{name: 'news', params: {title: story.title}}" aria-label="{{story.title}}">{{story.stitle}}</a>
                </li>
            </ul>
            <span class="show-more">
                <a v-link="{name: 'category', params: {title: storyblock.title}}">更多</a>
            </span>
        </div>
    </div>
    <hr>
